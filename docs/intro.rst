...
* Operating Systems: Linux, UNIX, macOS, native Windows, Windows with UNIX-like
  environments

* Python versions: 3.9 and higher

.. # Keep these Python versions in sync with pyproject.toml
...
* Install the {[ cookiecutter.pypi_package_name ]} package and its prerequisite
...
{[ cookiecutter.pypi_package_name ]} that is released on `PyPI`_.
This section describes how different versions of {[ cookiecutter.pypi_package_name ]}
can be installed.

* To install an older released version of {[ cookiecutter.pypi_package_name ]},
  Pip supports specifying a version requirement. The following example installs
  {[ cookiecutter.pypi_package_name ]} version 0.1.0
  from PyPI:

  .. code-block:: bash

      $ pip install {[ cookiecutter.pypi_package_name ]}==0.1.0
...
* If you need to get a certain new functionality or a new fix that is
  ...
  Git repository. The following example installs {[ cookiecutter.pypi_package_name ]}
  from the current code level in the master branch of the
  `{[ cookiecutter.github_repo ]} repository`_:

  .. code-block:: bash

      $ pip install git+https://github.com/{[ cookiecutter.github_org ]}/{[ cookiecutter.github_repo ]}.git@master#egg={[ cookiecutter.pypi_package_name ]}
...
You can verify that {[ cookiecutter.pypi_package_name ]} is installed correctly by
importing the package into Python (using the Python environment you installed
it to):

.. code-block:: bash

    $ python -c "import {[ cookiecutter.python_package_name ]}; print('ok')"
    ok
...
The version of the {[ cookiecutter.pypi_package_name ]} package can be accessed by
programs using the ``{[ cookiecutter.python_package_name ]}.__version__`` variable:

.. autodata:: {[ cookiecutter.python_package_name ]}._version.__version__

Note: For tooling reasons, the variable is shown as
``{[ cookiecutter.python_package_name ]}._version.__version__``, but it should be used as
``{[ cookiecutter.python_package_name ]}.__version__``.
...
{[ cookiecutter.pypi_package_name ]} package without encountering compatibility
